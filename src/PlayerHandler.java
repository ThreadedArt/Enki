...
public class PlayerHandler {
	public static final int maxPlayers = ServerCP.Maxplayers;
			...
			if (players[i].disconnected) {
				for (int i2 = 0; i2 < server.npcHandler.maxNPCs; i2++) {
					if (server.npcHandler.npcs[i2] != null && players[i] != null) {
						if (server.npcHandler.npcs[i2].followPlayer == players[i].playerId
								&& server.npcHandler.npcs[i2] != null)
	...
	public boolean saveGame(Player plr) {
		...
		try {
			ObjectOutputStream out = new ObjectOutputStream(
					new FileOutputStream("./savedGames/" + tempSave.playerName + ".dat"));
